...
export default function ProfilePage() {
  ...
  const handleLocalFiles = (files, type) => {
    Array.from(files).forEach((file) => {
      ...
      type === "resume" ? addResume(newFile) : addCoverLetter(newFile);
    });
  };
    ...
    <div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4">
        <UploadDropzone
          ...
          accept=".pdf,.doc,.docx"
          onFilesSelected={(files) => handleLocalFiles(files, "resume")}
        />
        <UploadDropzone
          label="Drag & drop your cover letter here"
          accept=".pdf,.doc,.docx"
          onFilesSelected={(files) => handleLocalFiles(files, "coverLetter")}
        />
      ...
      <div className="mb-6">
        ...
        {resumes.length === 0 ? <p className="text-gray-500">No resumes uploaded yet.</p> :
          <ul className="space-y-2">
            {resumes.map((file) => (
              <li key={file.id} className="flex items-center justify-between border p-3 rounded shadow hover:shadow-lg bg-white">
                <div className="flex items-center gap-3">{renderFileIcon(file)}<span>{file.name}</span></div>
                <div className="flex gap-2">
                  {file.url && <a href={file.url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">View</a>}
                  <button onClick={() => deleteResume(file.id)}><FaTrash className="text-red-500 hover:text-red-700" /></button>
                </div>
              </li>
            ))}
          </ul>
      ...
      <div className="mb-6">
        ...
        {coverLetters.length === 0 ? <p className="text-gray-500">No cover letters uploaded yet.</p> :
          <ul className="space-y-2">
            {coverLetters.map((file) => (
              <li key={file.id} className="flex items-center justify-between border p-3 rounded shadow hover:shadow-lg bg-white">
                ...
                <div className="flex gap-2">
                  {file.url && <a href={file.url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">View</a>}
                  <button onClick={() => deleteCoverLetter(file.id)}><FaTrash className="text-red-500 hover:text-red-700" /></button>
                </div>
              </li>
            ))}
          </ul>
